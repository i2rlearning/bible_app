...
let lastSavedMiniEditorState = "";

function serializeMiniEditorState(state) {
...
async function loadMiniEditorPage() {
  ...
  try {
    ...
    if (result.page && result.page.mini_editor_json) {
      ...
      lastSavedMiniEditorState = serializeMiniEditorState(savedState);
    } else {
      lastSavedMiniEditorState = serializeMiniEditorState(getMiniEditorState());
    }
...
async function saveMiniEditorPage() {
  ...
  const serializedState = serializeMiniEditorState(miniEditorJson);

  if (serializedState === lastSavedMiniEditorState) {
    return;
  }
  ...
  try {
    if (!flags.hasHighlights && !flags.hasDrawings && !flags.hasTextFormats) {
      ...
      lastSavedMiniEditorState = serializedState;
...
function startMiniEditorObserver() {
  ...
  miniEditorObserver = new MutationObserver((mutations) => {
    ...
    const hasUserContentChange = mutations.some((mutation) => {
      // resizeAnnotationLayer updates these attributes on the SVG container.
      // They are layout changes only and must not trigger a save.
      if (
