...
(function () {
  ...
  function buildReference(range) {
    ...
    const bookChapter =
      `${context.bookName} ${context.chapterNumber}`.trim();
    ...
    const markers =
      Array.from(
        bibleText.querySelectorAll(".v")
      )
        .map((marker) => ({
          verse:
            getVerseNumber(marker),
  ...
  function getSelectedText(range) {
    ...
    wrapper
      .querySelectorAll(
        ".v, .api-footnote-marker, .api-crossref-marker"
      )
      .forEach(
        (element) =>
          element.remove()
  ...
  function headerHtml(
    ...
    return `
      <div class="selection-study-menu-header">
        ...
        <span
          class="study-menu-bookmark-icon"
          aria-hidden="true"
        >
        ...
        <div>
          ...
          <span class="selection-study-menu-subtitle">
            ${escapeHtml(subtitle || "")}
          </span>
        </div>
  ...
  function studyButtonHtml(study) {
    ...
    return `
      <button
        type="button"
        class="selection-study-menu-button is-study"
  ...
  function renderStart() {
    ...
    if (!state.selection) {
      menu.insertAdjacentHTML(
        "beforeend",
        `
          <div class="selection-study-menu-status is-error">
            Select Scripture text first, then tap this button.
          </div>
        `
      );
    ...
    if (
      ...
      menu.insertAdjacentHTML(
        "beforeend",
        `
          <div class="selection-study-menu-status">
            You do not have any saved studies yet.
          </div>

          <button
            type="button"
            class="selection-study-menu-button is-primary"
            data-open-study-desk
          >
            Open Study Desk
          </button>
        `
    ...
    menu.insertAdjacentHTML(
      "beforeend",
      `
        <div class="selection-study-menu-label">
          Recent studies
        </div>
        ...
        <button
          type="button"
          class="selection-study-menu-button is-secondary"
          data-choose-study
        >
  ...
  function renderSearch() {
    ...
    menu.innerHTML = `
      <div class="selection-study-menu-top-row">
        ...
        <button
          type="button"
          class="selection-study-back-button"
          data-back
        >
          ‹ Back
        </button>
    ...
    const search =
      menu.querySelector(
        ".selection-study-search"
      );

    const results =
      menu.querySelector(
        ".selection-study-search-results"
      );
  ...
  function renderDestination(
    ...
    menu.innerHTML = `
      ...
      <div class="selection-study-reference">
        ${escapeHtml(
          state.selection.reference
        )}
      </div>
      ...
      <button
        type="button"
        class="selection-study-menu-button is-primary"
        data-mode="linked-scripture"
      >
        Linked Scripture

        <span>
          Add it to the study's Linked Scriptures list
        </span>
      </button>
      ...
      >
        ...
        <span>
          Append the selected words to the study notes
        </span>
      </button>
    `;
  ...
  function buildUpdate(
    ...
    let contentHtml =
      typeof study.contentHtml === "string"
        ? study.contentHtml
        : "";
  ...
  async function addToStudy(
    ...
    try {
      /*
       * Fetch the study fresh immediately before saving.
       * This avoids relying on an older copy from the initial
       * recent/search list.
       */
      const freshResponse =
        await fetch(
      ...
      if (
        ...
        throw new Error(
          freshResult.message ||
          "Could not load the selected study."
        );
      }
      ...
      if (
        ...
        throw new Error(
          saveResult.message ||
          "Could not update the selected study."
        );
      }

      const index =
        state.studies.findIndex(
          (item) =>
            item.id ===
            saveResult.study.id
        );
      ...
      menu.innerHTML =
        ...
        `
          <div class="selection-study-menu-status is-error">
            ${escapeHtml(
              error.message ||
              "Could not add the Scripture to the study."
            )}
          </div>
        ` +
  ...
  function bindTriggers() {
    document
      ...
      .forEach((trigger) => {
        ...
        /*
         * Capture the selected Bible range before the
         * toolbar button itself can take focus.
         */
  ...
  /*
   * Keep a lightweight memory of the selected Bible
   * range. This does not change or format the selection.
   */
  ...
  /*
   * Close the Study picker when the user taps elsewhere.
   */
  document.addEventListener(
    "pointerdown",
    (event) => {
      ...
      if (
        target.closest(
          "#selection-study-menu, [data-study-selection-trigger]"
        )
      ) {
        return;
      }
  ...
  /*
   * Escape closes the Study picker on desktop.
   */
  document.addEventListener(
    "keydown",
    (event) => {
  ...
  /*
   * Keep the dropdown attached to its button if the
   * viewport changes.
   */
