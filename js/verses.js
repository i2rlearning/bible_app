...
function updateCurrentPassageLocationLabels() {
  ...
  const mobileLabel = document.getElementById("mobile-current-passage-label");
  const mobileButton = document.getElementById("mobile-current-passage-button");
  const menuLabel = document.getElementById("menu-current-passage-label");

  if (mobileLabel) mobileLabel.textContent = shortLabel;

  if (mobileButton) {
    mobileButton.title = `Open ${fullLabel}`;
    mobileButton.setAttribute("aria-label", `Open ${fullLabel}`);
  }

  if (menuLabel) menuLabel.textContent = fullLabel;
}
...
function openCurrentPassagePickerFromCompactButton() {
  ...
  window.setTimeout(() => {
    if (versePassagePicker && typeof versePassagePicker.setOpen === "function") {
...
function initializePassagePicker() {
  if (!window.BibleSelector) {
    console.error("BibleSelector is unavailable.");
    return;
  }

  versePassagePicker = window.BibleSelector.createPassagePicker({
    root: document.getElementById("passage-picker"),
...
function configureMobilePassagePickerMenu() {
  const menuPassageLink = document.getElementById("openPassagePickerFromMenu");

  if (!menuPassageLink) return;

  menuPassageLink.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();

    if (typeof window.closeNav === "function") window.closeNav();
    if (typeof window.closeMobileToolbarMenus === "function") window.closeMobileToolbarMenus();
...
function buildBibleParams() {
  ...
  if (abbreviation) params.set("bibleAbbr", abbreviation);
  if (bibleName) params.set("bibleName", bibleName);
...
function configureVerseMenuLinks() {
  ...
  const searchLink = document.querySelector('.overlay-content a[href="./search.html"]');

  const bibleParams = buildBibleParams();

  if (homeLink) homeLink.href = `./index.html?${bibleParams.toString()}`;
  if (bookLink) bookLink.href = `./book.html?${bibleParams.toString()}`;

  if (chapterLink) {
    const chapterParams = new URLSearchParams(bibleParams);
    chapterParams.set("book", bibleBookID);
    if (bibleBookName) chapterParams.set("bookName", bibleBookName);
    chapterLink.href = `./chapter.html?${chapterParams.toString()}`;
  }

  if (searchLink) searchLink.href = `./search.html?${bibleParams.toString()}`;
}

function normalizeCurrentVerseUrl() {
  if (!bibleVersionID || !bibleChapterID) return;

  const normalizedParams = new URLSearchParams();
  normalizedParams.set("bible", bibleVersionID);

  if (abbreviation) normalizedParams.set("bibleAbbr", abbreviation);
  if (bibleName) normalizedParams.set("bibleName", bibleName);
  if (bibleBookID) normalizedParams.set("book", bibleBookID);
  if (bibleBookName) normalizedParams.set("bookName", bibleBookName);

  normalizedParams.set("chapter", bibleChapterID);

  const normalizedUrl = `./verse.html?${normalizedParams.toString()}`;
  const currentRelativeUrl = `${window.location.pathname.split("/").pop()}${window.location.search}`;
  const normalizedRelativeUrl = normalizedUrl.replace("./", "");

  if (currentRelativeUrl !== normalizedRelativeUrl) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener("readystatechange", function () {
      ...
      if (this.status < 200 || this.status >= 300) {
        reject(new Error(`Bible details request failed with status ${this.status}.`));
        return;
      }
    ...
    xhr.addEventListener("error", () => {
      reject(new Error("A network error occurred while loading Bible details."));
    });

    xhr.open("GET", `https://api.scripture.api.bible/v1/bibles/${encodeURIComponent(bibleId)}`);
    xhr.setRequestHeader("api-key", SCRIPT_API_KEY);
    xhr.send();
  });
}

async function initializeBibleIdentity() {
  if (!bibleVersionID) return;

  try {
    const bibleDetails = await getBibleDetails(bibleVersionID);

    abbreviation = bibleDetails.abbreviation || bibleDetails.abbreviationLocal || abbreviation || "";
    bibleName = bibleDetails.name || bibleDetails.nameLocal || abbreviation || bibleVersionID;

    const bibleTitle = document.getElementById("bible");
    const bibleFullNameElement = document.getElementById("biblefullname");

    if (bibleTitle) bibleTitle.textContent = abbreviation || "Bible";
    if (bibleFullNameElement) bibleFullNameElement.textContent = bibleName;

    normalizeCurrentVerseUrl();
    configureVerseMenuLinks();
  } catch (error) {
    console.error("Unable to retrieve Bible identity:", error);
    normalizeCurrentVerseUrl();
    configureVerseMenuLinks();
  }
}
...
function positionApiBibleFootnotePopup(marker, popup) {
  ...
  if (spaceBelow >= popupRect.height + margin) {
    ...
    top = markerRect.top - popupRect.height - 6;
  } else {
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape") closeApiBibleFootnotes();
});
...
// Page Initialization Route Guard
if (!bibleVersionID) {
  window.location.replace("./");
} else if (!bibleBookID) {
  const bookPageParams = buildBibleParams();
  window.location.replace(`./book.html?${bookPageParams.toString()}`);
} else if (!bibleChapterID) {
  const chapterPageParams = buildBibleParams();
  chapterPageParams.set("book", bibleBookID);
  if (bibleBookName) chapterPageParams.set("bookName", bibleBookName);
  window.location.replace(`./chapter.html?${chapterPageParams.toString()}`);
} else {
...
if (bibleTitle) bibleTitle.textContent = abbreviation || "Bible";
if (bookChapTitle) bookChapTitle.textContent = `${bibleBookName || book} ${chapter}`.trim();
if (bibleFullName) bibleFullName.textContent = bibleName || abbreviation || bibleVersionID;
...
function updateBibleZoomLayout() {
  ...
  displayText.style.overflowX = zoomedWidth > displayText.clientWidth ? "auto" : "hidden";
...
// Fetch and render chapter content
if (bibleChapterID) {
  getChapterText(bibleChapterID)
    ...
    .catch((error) => {
      ...
      const el = document.getElementById("bible-text");
      if (el) {
        el.innerHTML = "<p>Could not load chapter text. Please try again later.</p>";
      }
    });
}

// Navigation Seals & Touch Controls
...
function buildChapterUrl(chapterId) {
  ...
  if (abbreviation) url.searchParams.set("bibleAbbr", abbreviation);
  if (bibleName) url.searchParams.set("bibleName", bibleName);

  url.searchParams.set("book", currentBookId);
  if (bibleBookName) url.searchParams.set("bookName", bibleBookName);
...
function canGoPreviousChapter() {
  return Boolean(chapterNavigationState.previousChapter);
}

function canGoNextChapter() {
  return Boolean(chapterNavigationState.nextChapter);
}

function goToPreviousChapter() {
  if (!chapterNavigationState.previousChapter) return;
  window.location.href = buildChapterUrl(chapterNavigationState.previousChapter.id);
}

function goToNextChapter() {
  if (!chapterNavigationState.nextChapter) return;
  window.location.href = buildChapterUrl(chapterNavigationState.nextChapter.id);
}

function updateArrowCursorStates() {
  if (imageLeft) imageLeft.style.cursor = canGoPreviousChapter() ? "pointer" : "default";
  if (imageRight) imageRight.style.cursor = canGoNextChapter() ? "pointer" : "default";
}

async function initializeChapterArrows() {
  if (!window.BibleSelector || !bibleVersionID || !currentBookId || !bibleChapterID) {
    updateArrowCursorStates();
    return;
  }

  try {
    const chapters = await window.BibleSelector.loadChapters(bibleVersionID, currentBookId);
    chapterNavigationState = window.BibleSelector.getChapterState(chapters, bibleChapterID);
  } catch (error) {
    console.error("Could not initialize chapter navigation:", error);
  }
...
if (imageLeft) {
  imageLeft.addEventListener("mouseover", () => {
  ...
  imageLeft.addEventListener("mouseout", () => {
    imageLeft.src = "./img/orig_left_stamp.png";
  });

  imageLeft.addEventListener("click", goToPreviousChapter);
}

if (imageRight) {
  imageRight.addEventListener("mouseover", () => {
    if (canGoNextChapter()) {
  ...
  imageRight.addEventListener("mouseout", () => {
    imageRight.src = "./img/orig_right_stamp.png";
  });

  imageRight.addEventListener("click", goToNextChapter);
}
...
window.addEventListener("touchstart", (event) => {
  if (typeof window.isBibleDrawingActive === "function" && window.isBibleDrawingActive()) {
    initialTouchX = null;
    initialTouchY = null;
    return;
  }
...
window.addEventListener("touchend", (event) => {
  ...
  if (initialTouchX === null || initialTouchY === null || !event.changedTouches.length) {
    return;
  }
...
function isKeyboardNavigationBlocked(event) {
  if (versePassagePicker && versePassagePicker.isOpen()) return true;
  ...
  if (target.isContentEditable || target.closest('[contenteditable="true"], .ql-editor')) {
    return true;
  }
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "ArrowLeft" && canGoPreviousChapter()) {
    event.preventDefault();
    goToPreviousChapter();
  } else if (event.key === "ArrowRight" && canGoNextChapter()) {
    event.preventDefault();
    goToNextChapter();
  }
});
