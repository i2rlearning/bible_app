//***************************************************************************
// This file provides shared server-side functions to normalize, canonicalize,
...
// - Rejection of unknown Bible books and incomplete/invalid references
...
//
// Invalid examples:
//   jhn             -> invalid (missing chapter/verse)
//   banana 3:16     -> invalid (unknown Bible book)
//***************************************************************************
...
// Stable API.Bible book IDs. These are identifiers, not translated book names.
// Step 3 uses them for language/translation-independent Scripture lookups.
const SCRIPTURE_BOOK_ID_MAP = {
...
Object.entries(SCRIPTURE_BOOK_ID_MAP).forEach(([bookId, canonicalName]) => {
  SCRIPTURE_BOOK_ALIAS_MAP.set(getScriptureBookAliasKey(bookId), canonicalName);
});
...
function normalizeScriptureReference(value) {
  ...
  // Canonicalize a recognized book-only alias (for example "jhn" -> "John"),
  // but parseScriptureReference will still reject it because no chapter or
  // verse was supplied.
  if (!match) {
    return normalizeBookName(cleaned) || cleaned;
  }

  const bookName = normalizeBookName(match[1]);

  // Leave unknown book text unchanged here. The parser below will reject it.
  let reference = `${bookName || normalizeWhitespace(match[1])}${match[2]}`;
...
function parseScriptureReferenceList(value) {
  ...
  for (const rawSegment of rawSegments) {
    ...
    const key = parsed.normalizedReference.toLowerCase();
    if (seen.has(key)) {
      return null;
    }
